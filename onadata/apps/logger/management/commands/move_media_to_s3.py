...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            print(t("Missing necessary libraries. Try running: pip install -r"
                    "requirements/s3.pip"))
            sys.exit(1)

        default_storage = get_storage_class()()
        if default_storage.__class__ != s3.__class__:
            print(t("You must first set your default storage to s3 in your "
        ...
        for cls, file_field, upload_to in classes_to_move:
            print(t("Moving %(class)ss to s3...") % {'class': cls.__name__})
