...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('xls_filepath',
                            help=gettext_lazy("Path to the xls file"))

        parser.add_argument('username',
                            help=gettext_lazy("Username to publish the form to"))
    ...
    def handle(self, *args, **options):
        try:
            ...
            raise CommandError(t("You must provide the path to the xls file."))
        # make sure path exists
        if not xls_filepath or not os.path.exists(xls_filepath):
            raise CommandError(
                t("The xls file '%s' does not exist.") %
                xls_filepath)
