...
class Command(BaseCommand):
    args = 'username path'
    help = gettext_lazy("Import a zip file, a directory containing zip files "
    ...
    def handle(self, *args, **kwargs):
        if len(args) < 2:
            raise CommandError(t("Usage: <command> username file/path."))
        username = args[0]
        path = args[1]
        try:
        ...
        for dir, subdirs, files in os.walk(path):
            ...
            if "odk" in subdirs:
                ...
                subdirs.remove("odk")
                self.stdout.write(t("Importing from dir %s..\n") % dir)
