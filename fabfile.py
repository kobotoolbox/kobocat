...
def deploy(deployment_name, branch='master'):
    ...
    with cd(env.code_src):
        ...
        run("git reset HEAD kobocat")
        run("git checkout origin/%s" % branch)
        run("git submodule init")
        run("git submodule update")
        run("git remote add deploy https://github.com/onaio/kobocat-deploy.git"
            " || true")
        run("git fetch deploy")
        run("git read-tree --prefix=kobocat -i deploy/master")
